...
. Use the general testing checklist as a guide. 
. Test on the testing server and compare the behaviour with current documentation and the production server.
. Record the results of your testing in the checklist.

When an issue is identified:

. Create an RT ticket in the Testing queue.
. Link the RT ticket to the parent ticket for the specific testing project.
. Report only one issue on a ticket.
. Add any relevant EG-tags
...
. Include screenshots.
. Attempt to replicate the issue on an Evergreen community test server - https://wiki.evergreen-ils.org/doku.php?id=community_servers running the same 
...
. Include the steps to take to replicate the issue.
  ...
  current behaviour on the production server?
. Include screenshots.
. Attempt to replicate the issue on a Sitka upstream server - https://wiki.libraries.coop/doku.php?id=sitka:testing_server_info or an Evergreen community test server - https://wiki.evergreen-ils.org/doku.php?id=community_servers running the same 
version of Evergreen.
  .. If it can be replicated it's an upstream bug and needs to be xref::_reporting_a_bug[reported to Launchpad].
