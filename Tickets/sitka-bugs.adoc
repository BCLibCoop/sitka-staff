...
. Search https://bugs.launchpad.net/evergreen[Launchpad] for an existing bug.
.. If there is an existing bug - Select "This bug affects you"
.. If no existing bug, xref:_reporting_a_bug[report the bug].
. Add Launchpad bug # to RT ticket
. If indicated on Launchpad - include expected release in RT Ticket
...
. Include information about status (Launchpad / Sitka Development work)
