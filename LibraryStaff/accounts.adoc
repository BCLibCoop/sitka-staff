...
. Log in to Evergreen using a workstation registered to the requestors home library
. Go to *Circulation -> Register Patron*
.. create a user with profile using one of the Self Check profile groups and barcode/username of the form shortnameselfcheck.
... Use "BC Pub Self Check Login" for all BC Public Libraries
... Use "Self Check Login" for any other library
.. Enter a temporary password in the account.
. Use the library's email address in the account so they can do a password reset.
