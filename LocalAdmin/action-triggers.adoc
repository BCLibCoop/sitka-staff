...
Updating Action Trigger Templates to HTML
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
When the template is updated, its name should be changed to add *- HTML* as a suffix, for example, _ZSP - 3 Day Courtesy Notice - HTML_

The hyphen should not touch the text (a space before and after it), and HTML should be written in all caps. This way, filtering by title anything that _Contains_ HTML means results should retrieve all templates updated to HTML.
...
Here is a reference of https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements#text_content[HTML text content elements].
...
[NOTE]
======
https://en.wikipedia.org/wiki/List_of_XML_and_HTML_character_entity_references[Numeric character references] must be used for some characters.
...
Come to the library &amp;amp&semi; check out books!
