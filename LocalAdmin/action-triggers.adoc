...
For example, line breaks need to be coded in (i.e. &lt;br /&gt;), otherwise the text will be all stuck together on the same line. Another issue is that hyperlinks will no longer automatically link unless they are updated with HTML, &lt;a href=&quot;URL&quot;&gt;linked text&lt;/a&gt;

Additionally, https://en.wikipedia.org/wiki/List_of_XML_and_HTML_character_entity_references[numeric character references] must be used for some characters, e.g. if you want to the text display &lt; in the final notification, you must write &amp;lt&semi; in the template.
...
A div tag may be used to enclose the entire content of message. Here is an example of this in an email notification formatted in HTML:
